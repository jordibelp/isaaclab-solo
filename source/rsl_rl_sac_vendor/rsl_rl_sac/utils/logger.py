...
class Logger:
    ...
    def __init__(
        ...
        self.device = device
        self.git_status_repos = [rsl_rl_sac.__file__]
    ...
    def init_logging_writer(self) -> None:
        ...
        if self.log_dir is not None and not self.disable_logs:
            ...
            if self.logger_type == "neptune":
                from rsl_rl_sac.utils.neptune_utils import NeptuneSummaryWriter

                self.writer = NeptuneSummaryWriter(log_dir=self.log_dir, flush_secs=10, cfg=self.cfg)
            elif self.logger_type == "wandb":
                from rsl_rl_sac.utils.wandb_utils import WandbSummaryWriter
    ...
    def log(
        ...
        """
        if self.writer:
            # collection_size override is used by offpolicy runner in case log_interval is != 1
            collection_size = (
                collection_size_override
                if collection_size_override is not None
                else self.cfg["num_steps_per_env"] * self.num_envs * self.gpu_world_size
            )
            ...
            if len(self.rewbuffer) > 0:
                ...
                self.writer.add_scalar("Train/mean_reward", reward_mean, it)
                ...
                if self.logger_type == "wandb":
                    self.writer.add_scalar(
                        ...
                        reward_mean,
                        it,
                        walltime=self.tot_time,
                    )
                else:
                    self.writer.add_scalar(
                        "Train/mean_reward/time", reward_mean, int(self.tot_time)
                    )
                    self.writer.add_scalar(
            ...
            if len(self.rewbuffer) > 0:
                ...
                if terrain_level_mean is not None:
                    log_string += (
                        f"""{"Mean reward + terrain level:":>{pad}} """
                        f"""{statistics.mean(self.rewbuffer) + terrain_level_mean:.2f}\n"""
                    )
            ...
            if alpha is not None:
                log_string += f"""{"Alpha:":>{pad}} {alpha:.6f}\n"""
