...
class MLPModel(nn.Module):
    ...
    def __init__(
        ...
        if state_dependent_std and stochastic:
            self.mlp = MLP(self._get_latent_dim(), [2, output_dim], hidden_dims, activation, layer_norm=layer_norm)
        else:
            self.mlp = MLP(self._get_latent_dim(), output_dim, hidden_dims, activation, layer_norm=layer_norm)
    ...
    def forward(
        ...
        ..note::
            The `actions` parameter is unused in the base class. It is provided so that subclasses (e.g.,
            ``SACCriticModel``) can override ``forward()`` to accept actions without changing the call signature.
        """
