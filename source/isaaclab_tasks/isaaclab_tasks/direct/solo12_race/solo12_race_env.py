...
class Solo12RaceEnv(DirectRLEnv):
    ...
    def step(self, action: torch.Tensor):
        ...
        if len(reset_env_ids) > 0:
            ...
            # produces. Overriding step() opts out of the base class version of this block,
            # and every consumer checks for the key and degrades silently without it, so
            # SAC would treat each truncation as a true terminal state worth zero.
